    ...
    this.objects.forEach(obj => {
        if (typeof (tag) == 'string') {
            if (tag.startsWith('<') && tag.endsWith('>')) {
                ...
                obj.insertAdjacentElement('beforeBegin', $create(tag, properties, styles, attributes));                
            }
        }
        else if (typeof (tag) == 'object') {
            obj.insertAdjacentElement('beforeBegin', tag);
        }
    });
    return this;
}
    ...
    this.objects.forEach(obj => {
        if (typeof (tag) == 'string') {
            if (tag.startsWith('<') && tag.endsWith('>')) {
                ...
                obj.insertAdjacentElement('afterEnd', $create(tag, properties, styles, attributes));                
            }        
        }
        else if (typeof (tag) == 'object') {
            obj.insertAdjacentElement('afterEnd', tag);            
        }
    });
    return this;
}
    ...
    this.objects.forEach(obj => {
        if (typeof (tag) == 'string') {
            if (tag.startsWith('<') && tag.endsWith('>')) {
                ...
                obj.insertAdjacentElement('afterBegin', $create(tag, properties, styles, attributes));                
            }
        }
        else if (typeof (tag) == 'object') {
    ...
    this.objects.forEach(obj => {
        if (typeof (tag) == 'string') {
            if (tag.startsWith('<') && tag.endsWith('>')) {
                ...
                obj.appendChild($create(tag, properties, styles, attributes));                
            }
        }
        else if (typeof (tag) == 'object') {
            obj.appendChild(tag);
...
$root.prototype.css = function (name) {
    if (name == null) {
        if (this.objects.length == 0) {
            ...
            return this.objects.map(obj => obj.className).one();
        }
    }
    else if (name != '.') { // . 表示什么也不做
        this.objects.forEach(obj => {
            obj.className = name;
        });
    }
...
$root.prototype.icon = function(icon) {
    if (icon == null) {
        if (this.objects.length == 0) {            
            return null;
        }
...
String.prototype.toBoolean = function(defaultValue = false, object = null) {
    ...
    if (/^(true|yes|disabled|selected|enabled|1|ok|on|hidden|visible|)$/i.test(value)) {
        value = true;
    }
...
Event.bind = function (tag, eventName, func) {
    ...
    if (typeof(tag) == 'string') {
        ...
        if (tag.nodeType == 0 || document.components.has(tag.name) || Object.getOwnPropertyNames(tag.constructor.prototype).filter(n => n.startsWith('on')).map(n => n.toLowerCase()).includes(eventName)) {
            extension = true;
        }
            ...
            selector.split(',')
                ...
                .forEach(r => {
                    if (r.startsWith('#')) {
                        let s = $s(r);
                        if (s == null) {
                            throw new Error('Incorrect selector: ' + r);
                        }
                        if (s[method] == null) {
                            s = $x(r);
                        }
                        ...
                        if (x[method] != null)  {
                            if (item != '') {
                                x[method](item, value.$p(this));
                            }
                            else if (value != '') {
                                x[method](value.$p(this));
                            }
                            else {
                                x[method](...args);
                            }
...
Callout.Entity.prototype.locate = function() {
    switch(this.location) {
        case 0:  //left
            return $x('#__Callout').css("callout callout-right").leftside(this.reference, this.offsetX - 12, this.offsetY);
        case 1: //over
            return $x('#__Callout').css("callout callout-bottom").upside(this.reference, this.offsetX, this.offsetY - 12);
        case 2: //right
            return $x('#__Callout').css("callout callout-left").rightside(this.reference, this.offsetX + 12, this.offsetY);
        case 3: //under
            return $x('#__Callout').css("callout callout-top").downside(this.reference, this.offsetX, this.offsetY + 12);
        default:
            return true;
    }
}
...
Callout.Entity.prototype.show = function(seconds = 0) {
    $x('#__Callout').html(this.content).show().style('visibility', 'hidden');
    let i = 0;
    while (!this.locate()) {
    ...
    $x('#__Callout').style('visibility', 'visible');
    if (seconds > 0) {
...
$finish(function() {
    ...
    if ($s('#__Callout') == null) {
        $x(document.body).append('DIV', { id: '__Callout', className: 'callout', hidden: true });
        $x('#__Callout').bind('click', function() { this.style.display = 'none'; window.clearTimeout(Callout.Entity.$timer); });
    }

...
class HTMLTreeViewElement extends HTMLCustomElement {
    ...
    get loadDepth() {
        return $parseInt(this.getAttribute('load-depth'), 1);
    }
    ...
    collapseAll() {
        this.children.forEach(treeNode => treeNode.collapse());
    }
    ...
    expandTo(depth) {
        if (depth != 1 && this.hasChildNodes) {
            this.__toExpandedDepth = depth;
            this.__toExpanded[1] = new Map();
            for (let i = 0; i < this.children.length; i++) {                
                this.__toExpanded[1].set(this.children[i].guid, this.children[i]);
            }
            this.__expandToExpanded(1);
        }
        else if (!this.#preloaded) { 
            this.loadTo(this.loadDepth);         
        }
    }

    __toExpanded = [];  //depth -> Map(guid, treeNode)
    __toExpandedDepth = 1;

    //宽度优先，比深夜优先减少很多递归
    __expandToExpanded (depth) {
        [...this.__toExpanded[depth].values()]
            .forEach(treeNode => {
                treeNode.expand(false, function() {
                    //展开一个移除一个
                    this.treeView.__toExpanded[this.depth].delete(treeNode.guid);
                    
                    if (this.depth < this.treeView.__toExpandedDepth || this.treeView.__toExpandedDepth == 0) {
                        //添加子节点到待展开队列
                        if (this.treeView.__toExpanded[this.depth + 1] == null) {
                            this.treeView.__toExpanded[this.depth + 1] = new Map();
                        }
                        for (let i = 0; i < this.children.length; i++) {       
                            if (this.children[i].value != 'LOADING__') {
                                this.treeView.__toExpanded[this.depth + 1].set(this.children[i].guid, this.children[i]);
                            }                                                    
                        }
                    }
                    //如果同级节点都已经展开完成
                    if (this.treeView.__toExpanded[this.depth].size  == 0) {
                        if ((this.depth < this.treeView.__toExpandedDepth || this.treeView.__toExpandedDepth == 0) && this.treeView.__toExpanded[this.depth + 1].size > 0) {
                            //还有下一级节点，继续展开
                            this.treeView.__expandToExpanded(this.depth + 1);
                        }
                        else if (!this.treeView.preloaded) {
                            this.treeView.loadTo(this.treeView.loadDepth); 
                        }
                    }
                });
            });
    ...
    __toLoaded = []; //depth -> Map(guid, treeNode)
    __toLoadedDepth = 1;

    loadTo(depth) {
        if (depth != 1 && this.hasChildNodes) {
            this.__toLoadedDepth = depth;
            this.__toLoaded[1] = new Map();
            for (let i = 0; i < this.children.length; i++) {                
                this.__toLoaded[1].set(this.children[i].guid, this.children[i]);
            }
            this.__loadToLoaded(1);
        }
        else if (!this.#preloaded) { 
    ...
    __loadToLoaded (depth) {
        [...this.__toLoaded[depth].values()]
            .forEach(treeNode => {
                treeNode.load(false, function() {
                    //加载一个移除一个
                    this.treeView.__toLoaded[this.depth].delete(treeNode.guid);
                    
                    if (this.depth < this.treeView.__toLoadedDepth || this.treeView.__toLoadedDepth == 0) {
                        //添加子节点到待展开队列
                        if (this.treeView.__toLoaded[this.depth + 1] == null) {
                            this.treeView.__toLoaded[this.depth + 1] = new Map();
                        }
                        for (let i = 0; i < this.children.length; i++) {       
                            if (this.children[i].value != 'LOADING__') {
                                this.treeView.__toLoaded[this.depth + 1].set(this.children[i].guid, this.children[i]);
                            }                                                    
                        }
                    }
                    //如果同级节点都已经加载完成
                    if (this.treeView.__toLoaded[this.depth].size  == 0) {
                        if ((this.depth < this.treeView.__toLoadedDepth || this.treeView.__toLoadedDepth == 0) && this.treeView.__toLoaded[this.depth + 1].size > 0) {
                            //还有下一级节点，继续展开
                            this.treeView.__loadToLoaded(this.depth + 1);
                        }
                        else if (!this.treeView.preloaded) {
            ...
            if (node != null) {
                if (names.length > 1) {
                    ...
                    if (node.loaded) {
                        ...
                        node.expand(false, function () { this.__selectNodeByPath(names); });
                    }
                }
                else {
                    node.select(false);
...
HTMLCustomElement.defineEvents(HTMLTreeViewElement.prototype, {
    ...
    onnodeselectandload: 'onNodeSelectedAndLoaded', //当节点被选择且加载完成后触发，在 onNodeSelected 之后
...
class HTMLTreeNodeElement extends HTMLCustomElement { 
    ...
    #guid = 'TreeNode_' + String.shuffle(9);
    ...
    #appendLoadingNode (slot) {
        this.insertBefore({
            ...
            text: 'Loading...',
            value: 'LOADING__',
            icon: this.treeView.contentLoadingImageURL,
            draggable: false,
            droppable: false
        }, slot);
    }
    ...
    populate () {
        ...
        this.setAttribute('index', (this.parentNode ?? this.treeView).children.length);
    ...
    expand (triggerEvent, callback) {
        ...
        if (this.treeView != null && !this.#expanded) {
            ...
            if (!this.#loaded) {
                ...
                this.load(function () {
                        if (this.#expanding) {
                            this.#completeExpanding(triggerEvent, callback); 
                        }
                    });
            }
            else {
    ...
    collapse(triggerEvent) {
        if (this.treeView != null) {
            ...
            if (triggerEvent !== false) { 
                this.treeView.dispatch('onNodeCollapsed', { 'collapsedNode': this, ev: triggerEvent || null }); 
            }  
        }
    }

    #completeExpanding (triggerEvent, callback) {
            ...
            this.#slots.forEach(slot => {                    
                ...
                slot.template
                    ...
                    .load(function() {
                        ...
                        this.#slotsToLoad --;
                        if (this.#slotsToLoad == 0) {                                
                            this.#completeLoading(triggerEvent, callback);
                        }
                    });
            });
        }
    ...
    #completeLoading (triggerEvent, callback) {
        this.#loading = false;
        this.#loaded = true;
        ...
        if (triggerEvent !== false) {
            ...
            if (this.selected) {
                this.treeView.dispatch('onNodeSelectedAndLoaded', { 'loadedNode': this });
            }
        }        
    }
    ...
    remove() {
        //重置 index
        let parent = (this.parentNode ?? this.treeView);
        parent.children.splice(this.index, 1);
        for (let i = this.index; i < parent.children.length; i++) {
